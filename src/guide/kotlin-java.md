...
configuration file is unnecessary. Instead, one has to deal with a few
imports:
...
When registering a domain, there are a few more variables: The account can
only be registered to an existing domain. Also, an account typically has to
have a key pair. So if e.g. _alice@wonderland_ was registering an account
for _white_rabbit@looking_glass_ she should provide his public key.
...
the account. Again, it’s important to note that we are using
...
_time#looking_glass_ that was not mintable. Due to a technical limitation
we cannot prevent that asset from being minted. However, we can ensure that
the late bunny is always late, by noting that _alice@wonderland_ can mint
time, but only to her account initially.

If she tried to mint an asset that was registered using a different client,
which was not mintable, this attempt would have been rejected, _and alice
...
supports only some features. Fortunately these features are not
used/covered in the Rust tutorial either, so as to keep the walkthrough
