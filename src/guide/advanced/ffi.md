...
A Rust type is converted into a robust `repr(C)` type that can cross FFI
...
mistakes, you must ensure the correctness of the program on your end.
...
- For setting the `field_name` field in `StructName` struct, the FFI
  function name would be `StructName__set_field_name`.
- For getting the `field_name` field in `StructName` struct, the FFI
